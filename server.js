...
app.post('/upload', upload.single('file'), async (req, res) => {
    try {
        ...
        // INSTANT UPDATE: Set total chunks immediately so the frontend can show 0/18
        ...
        for (let i = 0; i < totalChunks; i++) {
            ...
            // TICK UP: update after each successful Discord post
...
app.delete('/files/:id', async (req, res) => {
    ...
    if (file) {
        for (const chunk of file.chunks) {
            try { await axios.delete(`${WEBHOOK_URL}/messages/${chunk.msgId}`); } catch (e) {}
        }
